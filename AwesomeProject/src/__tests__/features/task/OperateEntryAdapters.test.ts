...
function sessionFor(taskId: string, sessionRevision: number): ResolvedOperateSessionV1 {
  return {
    schemaVersion: 1,
    taskId,
    sessionRevision,
...
describe('parseTaskExecutionData', () => {
  it('accepts exactly {taskId, sessionRevision}', () => {
    expect(parseTaskExecutionData('{"taskId":"task-1","sessionRevision":7}')).toEqual({
      taskId: 'task-1',
      sessionRevision: 7,
    });
  });
  ...
  it('rejects extra keys, stale/invalid revisions, blank ids and malformed JSON', () => {
    expect(parseTaskExecutionData('{"taskId":"t","sessionRevision":1,"model":{}}')).toBeNull();
    expect(parseTaskExecutionData('{"taskId":"t"}')).toBeNull();
    expect(parseTaskExecutionData('{"taskId":"","sessionRevision":1}')).toBeNull();
    expect(parseTaskExecutionData('{"taskId":"t","sessionRevision":0}')).toBeNull();
    expect(parseTaskExecutionData('{"taskId":"t","sessionRevision":1.5}')).toBeNull();
    expect(parseTaskExecutionData('{"taskId":"t","sessionRevision":-3}')).toBeNull();
    expect(parseTaskExecutionData('not json')).toBeNull();
  });

  it('serializes only the two minimal keys with no credential or instruction', () => {
    const serialized = serializeHeadlessTaskData({taskId: 'task-1', sessionRevision: 7});
    expect(JSON.parse(serialized)).toEqual({taskId: 'task-1', sessionRevision: 7});
    expect(serialized).not.toMatch(/instruction|model|apiKey|baseUrl|secretRef|screenshot|response/i);
  });
});
...
describe('foreground and Headless entry adapters', () => {
  ...
  it('drives one immutable session through the same runner from both entry points', async () => {
    ...
    const runner = {run: jest.fn().mockResolvedValue({kind: 'success'} as OperateTaskOutcome)};
    ...
    const runtime = {
      ...
      claim: (taskId: string, sessionRevision: number, owner: 'foreground' | 'headless') =>
    ...
    const headless = new HeadlessTaskExecutionAdapter({runtime: headlessRuntime, runner});
    const hlResult = await headless.run(serializeHeadlessTaskData({taskId: 'task-2', sessionRevision: 1}));
  ...
  it('foreground never runs when session creation is blocked', async () => {
    ...
    const runtime = {
      createSession: jest.fn().mockResolvedValue({ok: false, code: 'local_model_not_ready'}),
      claim: jest.fn(),
    };
    const foreground = new ForegroundTaskExecutionAdapter({
      runtime,
  ...
  it('Headless rejects an invalid payload without touching the runner', async () => {
    ...
    const result = await headless.run('{"taskId":"t","sessionRevision":1,"apiKey":"x"}');
  ...
  it('Headless rejects a stale revision as a replay', async () => {
    ...
    const headless = new HeadlessTaskExecutionAdapter({runtime, runner});
    const result = await headless.run(serializeHeadlessTaskData({taskId: 'task-1', sessionRevision: 2}));
