...
**已接入：** 用户确认使用 Elel。首页 WebView 加载 3D VRM（眨眼 / 拖转 / 情绪），加载失败回退半身图。
...
与 NoNo 全页视觉的融合计划（壳是 NoNo，脸是 Elel）：[`docs/superpowers/plans/2026-08-19-fused-nono-elel-plan.md`](../docs/superpowers/plans/2026-08-19-fused-nono-elel-plan.md)。不要把 PR #2 的旧蓝灰首页直接合进 master。
...
**当前落地（用户已要求 Elel 必须是 3D）：** 首页默认 WebView 播内置 `elel.vrm`；WebGL 失败才回退半身图。不要再把「默认只有 2.5D」当成产品结论。
