...
外观不进底栏，不进「能力」（那是操作/交代/OpenClaw/隐私），不塞进首页舞台（首页只负责说话）。

放在 **设置 → 陪伴** 分组里，紧挨「对话模型」下面一行：

- 标题：`角色外观`
- 说明：`只换首页角色长什么样，不改说话用的模型`
- 进入栈页 `AvatarLooks`（有返回）

该页不是纯列表。上半是和首页同一套 `NonoAvatar3D` 预览当前选中项（未下载时只预览内置 3D，不偷偷下载）；下半横向滑动角色卡：内置项永远第一、不可删。未下载的卡显示体积和「下载」；已下载的卡显示「使用」。点下载仍要确认（Wi-Fi / 移动网络都要），有进度。失败回滚预览到内置 3D。

其它项来自 `AvatarArtifactPins`。用户点选后：
...
失败：不改 `activeId`（若已是该 id 则改回 `builtin`），删除损坏目录，预览和首页回到内置 3D。
