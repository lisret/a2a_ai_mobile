 ...
 * 无障碍操作模块
 * 负责执行各种操作（点击、滑动、输入等）
 * 支持 Android（AccessibilityService + ADB 回退）和 iOS（UIAccessibility）
 */
...
export async function executeActionWithFallback(
  ...
  if (Platform.OS === 'ios') {
    if (!AccessibilityActionModule) {
      throw new Error('iOS 原生模块未找到');
    }
...
export async function performClickAction(
  ...
  await executeActionWithFallback(
    ...
    async () => { const adbService = await getAdbService(); await adbService.performClick(x, y); },
    getSettingsService
  );
}
...
export async function performLongPressAction(
  ...
  await executeActionWithFallback(
    ...
    async () => { const adbService = await getAdbService(); await adbService.performLongPress(x, y); },
    getSettingsService
  );
}
...
export async function performDoubleTapAction(
  ...
  await executeActionWithFallback(
    ...
    async () => { const adbService = await getAdbService(); await adbService.performDoubleTap(x, y); },
    getSettingsService
  );
}
...
export async function performSwipeAction(
  ...
  await executeActionWithFallback(
    ...
    async () => { const adbService = await getAdbService(); await adbService.performSwipe(startX, startY, endX, endY); },
    getSettingsService
  );
}
...
export async function performTextInputAction(
  ...
  await executeActionWithFallback(
    ...
    async () => {
      ...
      if (!success) console.warn('[文本输入] 无障碍服务返回 false，输入可能失败');
      return success;
    },
    async () => {
...
export async function performBackAction(
  ...
  await executeActionWithFallback(
    ...
    async () => { const adbService = await getAdbService(); await adbService.performBack(); },
    getSettingsService
  );
}
...
export async function performHomeAction(
  ...
  if (Platform.OS === 'ios') {
    if (!AccessibilityActionModule) throw new Error('iOS 原生模块未找到');
    const success = await AccessibilityActionModule.performHome();
    if (!success) throw new Error('Home操作执行失败');
    return;
  }
  ...
  try {
    ...
    try {
      ...
      throw new Error(`Home操作失败（无障碍和ADB都失败）: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
