...
**为什么选 OpenClaw？—— SubAgent + Skill 挂载**

OpenClaw 的杀手特性是 **SubAgent 架构**：每个 SubAgent 是一个独立的执行单元，可以挂载指定的 **Skill**（技能）。这意味着：

```
网关收到任务 →

  ① "A 设备执行点赞任务" → 创建 SubAgent-A，挂载「点赞 Skill」
      SubAgent-A 知道点赞的步骤模板、目标 app、异常处理策略

  ② "B 设备执行自动回复任务" → 创建 SubAgent-B，挂载「自动回复 Skill」
      SubAgent-B 知道回复的文案策略、触发条件、频率控制

每个 SubAgent 只关心自己擅长的领域，互不干扰。
网关根据任务类型匹配对应 Skill 的 SubAgent → 下发到最合适的设备执行。
```

这种设计的好处：
- **任务与设备解耦**：Skill 定义"怎么干"，设备只负责"动手"，同一个 Skill 可以调度到任意空闲设备
- **Skill 可复用**：点赞 Skill 写一次，A 设备用完 B 设备接着用
- **并行隔离**：多个 SubAgent 同时跑不同任务，互不冲突
- **扩展简单**：新增任务类型 = 新增一个 Skill，不需要改调度逻辑
...
| 模式 | 范式 | 决策位置 | 适合场景 |
|------|------|---------|---------|
| 🔵 **本地 Agent** | ReAct（感知→推理→行动→观察，逐步循环） | 手机端（Agent 闭环 + 直接调模型 API） | 单设备日常使用、实时交互 |
| 🟣 **OpenClaw 远程** | Plan-Execute 变体（网关规划 → 客户端执行 → 截图验证） | 云端网关（规划 + 模型调用集中） | 集群调度、批量任务、企业集成 |
...
│          OpenClaw 网关 (大脑)          │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ │
│  │SubAgent A│ │SubAgent B│ │SubAgent C│ │  ← 每个挂载不同 Skill
│  │(点赞Skill)│ │(回复Skill)│ │(采集Skill)│ │
│  └────┬────┘ └────┬────┘ └────┬────┘ │
│       │  任务规划 · 上下文管理 · 模型调用  │
└───────┼──────────┼──────────┼─────────┘
...
两套模式的设计取舍：**本地模式决策层在 App 内，直接调用模型 API，灵活可控；远程模式决策层在网关，客户端只执行和截图，便于统一管控和集群调度。** 两种模式都需要网络连接云端模型。区别在于 Agent 的"大脑"放在哪里——手机端还是网关。
...
本项目采用 [CC BY-NC 4.0 (知识共享 署名-非商业性使用 4.0)](./LICENSE) 协议。

**仅供个人学习、研究与交流使用，严禁任何形式的商业用途。**
