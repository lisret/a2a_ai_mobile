      ...
      {steps.map(step => {
        ...
        return (
          <View key={step.step} style={styles.item}>
            ...
            <Text style={styles.state}>{isDone ? 'DONE' : isActive ? 'NOW' : 'NEXT'}</Text>
          </View>
        );
      })}
    </View>
...
const styles = StyleSheet.create({
  ...
  indexComplete: {backgroundColor: '#dcf2ec'},
  indexCurrent: {backgroundColor: COLORS.violet},
  indexText: {color: '#777a87', fontSize: 10, fontWeight: '800'},
  indexCompleteText: {color: COLORS.success},
  indexCurrentText: {color: '#ffffff'},
  copy: {flex: 1},
  title: {color: COLORS.text.primary, fontSize: 12, fontWeight: '700'},
  desc: {marginTop: 3, color: COLORS.text.secondary, fontSize: 9},
  state: {color: COLORS.text.secondary, fontSize: 9, fontWeight: '700'},
});
