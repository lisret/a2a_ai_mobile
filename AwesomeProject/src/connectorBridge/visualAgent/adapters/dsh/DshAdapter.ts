// DeepSeek Harness Visual Agent adapter. Disabled by default and conservative by
// construction: it opens only the transport-neutral `VisualAgentUpstreamPort`
// (never spawning `dsh` or opening a product endpoint), binds strictly to
// `product: 'deepseek-harness'`, and grants a capability only when the upstream
// negotiation explicitly reports it. Requested-but-not-negotiated capabilities
// fail closed with a generic `visual_agent_capability_unsupported` before send.
...
import {validateDshBinding, type DshBindingV1} from './DshBinding';
...
class DshExecutionPort implements VisualAgentExecutionPort {
  ...
  async connect(
    ...
    let binding: DshBindingV1;
    try {
      const raw = await this.bindings.read(profile.connector.bindingId, 'dsh');
      binding = validateDshBinding(raw);
    } catch (error) {
    ...
    // Disabled-by-default and unknown/blank versions fail closed before any IO.
    if (!binding.enabled || binding.version.trim().length === 0) {
      this.state = {status: 'failed', errorCode: 'visual_agent_invalid_profile'};
  ...
  private nextRequestId(): string {
    this.requestSeq += 1;
    return `dsh-req-${this.requestSeq}`;
  }
...
export class DshAdapter implements VisualAgentToolAdapter {
  ...
  validateBinding(input: unknown): DshBindingV1 {
    return validateDshBinding(input);
  }

  create(_profile: VisualAgentProfileV1): VisualAgentExecutionPort {
    return new DshExecutionPort(this.bindings, this.upstream);
  }
}
